...
$.http.get({url: "https://api.xcurrency.com/rate/mid/latest?apiKey=4c083ce9-801d-4754-8cdf-7424690e05d2&quote=CNY"})
    .then((response) => {
        const data = JSON.parse(response.body);
        const source = currencyNames[quote];

        const info = Object.keys(currencyNames).reduce((accumulator, key) => {
            let line = "";
            if (key !== quote && data.rates.hasOwnProperty(key)) {
                const quote = parseFloat(data.rates[key]);
